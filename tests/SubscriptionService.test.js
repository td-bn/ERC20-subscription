...
describe("Subscription Service", () => {
    ...
    const createPlan = async () => 
        await service.connect(merchant).createPlan(token.address, MONTH.toString(), 100);
    ...
    describe("Adding plans", () => {
        it("should add a plan if is a valid plan", async () => {
            ...
            expect(plan.frequency).equals(MONTH.toString());
        ...
        it("should fail if token address is invalid", async () => {
            await expect(service.connect(merchant).createPlan(ethers.constants.AddressZero, MONTH.toString(), 100))
        ...
        it("should fail if cost is invalid", async () => {
            await expect(service.connect(merchant).createPlan(token.address, MONTH, 0))
